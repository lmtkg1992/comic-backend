...
use crate::models::stories::stories::{ Stories, ListStories ,LastChapter};
...
//      println!("{:#?}", story);
    ...
    async fn get_last_chapter(&self, story_id: &str, total_chapters: i64) -> Option<LastChapter> {
        ...
        let chapter_filter = doc! { "story_id": story_id, "ordered": total_chapters };
        ...
        chapter_result.map(|chapter| LastChapter {
            url_key: chapter.get_str("url_key").unwrap().to_string(),
            title: chapter.get_str("short_title").unwrap().to_string(),
        })
    }
}